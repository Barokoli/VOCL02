...
static int GetBlock(int x, int y, int z,__global float* r, int noiseCount, int noiseSize){
  //return z > 60? 0:1;
    ...
    value = ((float)((float)(127-z)/64.0f)-1.0f)+value*0.07;
...
static float castRay(float4 origin, float4 dir,__global int *chunk,int chunkL){
  float depth = 0;
...
__kernel void Raycaster (__write_only image2d_t frameBuffer,__global float* vectors, __global int* chunk,int chunkL){
//CamPos 0,1,2 Dir 3,4,5 Up 6,7,8 right 9,10,11 scPos 12,13,14 fov 15 clipnear 16 clipFar 17
	const int2 pos = {get_global_id(0), get_global_id(1)};

  //float d = EvaluateBlock(63,pos.x%128,pos.y%128,chunk,chunkL)>127?1:0;

  float d = castRay((float4)(vectors[0],vectors[1],vectors[2],0),
        (float4)( vectors[12]+vectors[9] * pos.x + vectors[6] * pos.y,
                vectors[13]+vectors[10] * pos.x + vectors[7] * pos.y,
                vectors[14]+vectors[11] * pos.x + vectors[8] * pos.y,0.0f),
        chunk,
        chunkL)/127.0f;

    //float d = EvaluateBlock(vectors[0],vectors[1],vectors[2],chunk,chunkL)>127?1:0;

        //float b = EvaluateBlock((int)vectors[0],(int)vectors[1],(int)vectors[2],chunk,chunkL);
	  write_imagef (frameBuffer, (int2)(pos.x, pos.y), (float4)(d,d,d,1.0f));

    /*const int2 pos = {get_global_id(0), get_global_id(1)};
    //float c = (pos.x/1024.0f) > sin((float)t*0.05f) ? 1.0f : 0.0f;//pos.x > sin((float)t*0.003f) ? 1.0f : 0.0f;
    write_imagef (frameBuffer, (int2)(pos.x, pos.y), (float4)(pos.x,pos.y,r[pos.x],1.0f));
    float4 cPos = (float4)();
    //Cast per pixel
    castRay(cPos, float4 dir,__global int *chunk,int chunkL){*/
}
