...
void initChunk(){
   ...
   // writeOut(aworld.tmpChunkData,aworld.tmpChunkDataSize);
    ...
    //writeOut(aworld.tmpScanLengthData,aworld.tmpScanLengthDataSize);


    int dChunkSize = aworld.tmpDataSizeCube/aworld.MaxChunkSize;
    //std::cout << dChunkSize << " : d Chunk Size" << std::endl;
    //CL_Handler.tmpScanLengthData[0] --;
    uint tmpFirst = 0;
    uint tmpSecond = 0;
    for(int iter = 0; iter < dChunkSize; iter++){
        activeChunk.MemLength += (int)aworld.tmpScanLengthData[iter];
   ...
   // writeOut(aworld.tmpScanLengthData,aworld.tmpScanLengthData);
    //Debug.Log("Memory Length: "+MemLength);
    //t = Time.realtimeSinceStartup;
    CheckErrorCL (clEnqueueWriteBuffer(queue, aworld.tmpScanLengthDataCLMem, CL_TRUE, 0, sizeof(uint) * aworld.tmpScanLengthDataSize, aworld.tmpScanLengthData, 0, NULL, NULL),__LINE__);
    clFinish(queue);
    cl_int error;
    activeChunk.chunkMem = new uint[activeChunk.MemLength];

    activeChunk.chunkCLMem = clCreateBuffer (context, CL_MEM_READ_WRITE | CL_MEM_COPY_HOST_PTR,
                                    sizeof (uint) * activeChunk.MemLength, activeChunk.chunkMem, &error);
    ...
    SetKernelArgMem(aworld.chunkMemCpyKernel,2,activeChunk.chunkCLMem);
    ...
    clFinish(queue);
    CheckErrorCL (clEnqueueReadBuffer(queue, activeChunk.chunkCLMem, CL_TRUE, 0, sizeof(uint)*activeChunk.MemLength, activeChunk.chunkMem, 0, NULL, NULL),__LINE__);
    clFinish(queue);
    //writeOut(ac.chunkMem,ac.MemLength);
    std::cout << activeChunk.MemLength << " : file length at line:" << __LINE__ << std::endl;
...
void checkInputs(GLFWwindow* window){
	if(glfwGetMouseButton(window, GLFW_MOUSE_BUTTON_LEFT) == GLFW_PRESS){
		if(mainCamera.drag){
			mainCamera.rotateSS(mainCamera.dragStart[0]-mousePosX,mainCamera.dragStart[1]-mousePosY);
			mainCamera.dragStart[0] = mousePosX;
			mainCamera.dragStart[1] = mousePosY;
		}
	}
	if(glfwGetKey(window, GLFW_KEY_LEFT_SHIFT) == GLFW_PRESS){
		mainCamera.speed = 2.0f;
	}
	...
	if(glfwGetKey(window, GLFW_KEY_W) == GLFW_PRESS){
		mainCamera.pos += mainCamera.dir * mainCamera.speed;
	}
	if(glfwGetKey(window, GLFW_KEY_A) == GLFW_PRESS){
		mainCamera.pos -= mainCamera.right*mainCamera.width * mainCamera.speed;
	}
...
void mouse_button_callback(GLFWwindow* window, int button, int action, int mods)
{
    if (button == GLFW_MOUSE_BUTTON_LEFT && action == GLFW_PRESS){
    	if(!mainCamera.drag){
    		mainCamera.drag = true;
    		mainCamera.dragStart[0] = mousePosX;
    		mainCamera.dragStart[1] = mousePosY;
    	}
    }else if (button == GLFW_MOUSE_BUTTON_LEFT && action == GLFW_RELEASE){
    	if(mainCamera.drag){
    		mainCamera.drag = false;
    		mainCamera.rotateSS(mainCamera.dragStart[0]-mousePosX,mainCamera.dragStart[1]-mousePosY);
    	}
    }
}
...
int main ()
{
    ...
    window = glfwCreateWindow( mainCamera.width, mainCamera.height, "VOCL", NULL, NULL);
    if( window == NULL ){
    ...
    CheckError(glGetError(), __LINE__);*/
	unsigned char * tmp_data = new unsigned char [mainCamera.width*mainCamera.height*4];
    ...
    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_NEAREST);
    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_NEAREST);
    glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA8, mainCamera.width, mainCamera.height, 0, GL_RGBA, GL_UNSIGNED_BYTE, tmp_data);
	...
	program = CreateProgram (LoadKernel ("src/kernels/image.cl"),
		context);
    if(exists_test("src/kernels/image.cl")){
    ...
    std::size_t size [3] = { mainCamera.width, mainCamera.height, 1 };

    mainCamera.kernelParamsCLMem = clCreateBuffer (context, CL_MEM_WRITE_ONLY | CL_MEM_COPY_HOST_PTR,
    	                                    sizeof (float) * 18, mainCamera.kernelParams, &error);
    ...
    do{
        ...
        CheckErrorCL (clEnqueueWriteBuffer(queue, mainCamera.kernelParamsCLMem, CL_TRUE, 0, sizeof(float) * 18, mainCamera.kernelParams, 0, NULL, NULL),__LINE__);
        clFinish(queue);
        // Setup the kernel arguments


        clSetKernelArg (kernel, 0, sizeof (cl_mem), &clMemImg);

        SetKernelArgMem(kernel,1,mainCamera.kernelParamsCLMem);
        SetKernelArgMem(kernel,2,activeChunk.chunkCLMem);
        SetKernelArgValue (kernel, 3, activeChunk.MemLength,sizeof(int));
